...
Storage::Storage(thrust::host_vector<unsigned int> shape, float value = 0)
    : shape(shape) {
  unsigned int size = thrust::reduce(shape.begin(), shape.end(), (unsigned int)1,
                                    thrust::multiplies<unsigned int>());
  this->data.resize(size, value);
}

Storage::Storage(std::initializer_list<unsigned int> shape, float value) {
  this->shape = thrust::device_vector<unsigned int>(shape.begin(), shape.end());
  unsigned int size =
